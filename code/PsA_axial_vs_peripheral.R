...
df_alpha = read.table('/Users/KevinBu/Desktop/clemente_lab/Projects/ampaim/outputs/jobs27/df_alpha_psa.tsv', 
                      sep = '\t', header = TRUE, row.names = 1, check.names = FALSE,
                      na.strings = "NA")
df_alpha$Involvement[df_alpha$Involvement == "axial"] <- "Axial"
df_alpha$Involvement[df_alpha$Involvement == "peripheral"] <- "Peripheral"
...
stats.table.all[1,2] <- kruskal.test(df_alpha[,2] ~ Involvement, data = df_alpha)$p.value
stats.table.all[1,3] <- oneway.test(df_alpha[,2] ~ Involvement, data = df_alpha)$p.value

# save
ft.all = paste(dir, "alpha_psa_stats.csv", sep = "")
...
col1 <- c('#D3A6C4', '#6A0D91')

# variable of interest
a <- 'shannon_entropy'

# create filenames
filename_box.plot = paste("alpha_psa_box.plot.pdf", sep = "_")  

# rewrite order of factors
d.final <- df_alpha[, c("Involvement","shannon_entropy")]
dx.order = c("Peripheral", "Axial")
d.final$Involvement <- factor(d.final$Involvement, levels = dx.order)
...
# create plot
p <- ggplot(d.final, aes(x = Involvement, y = shannon_entropy, fill = Involvement)) +
  geom_boxplot() +
  bkg + 
  # theme_minimal() +
  theme(legend.position = "none") + 
  labs(x = "Involvement", y = "Shannon Entropy") +
...
# load data
df <- read.delim(file="/Users/KevinBu/Desktop/clemente_lab/Projects/ampaim/outputs/jobs27/bray_curtis_pcoa_psa.tsv",
                 row.names=1)

df$Involvement[df$Involvement == "axial"] <- "Axial"
df$Involvement[df$Involvement == "peripheral"] <- "Peripheral"

# order factors for legend
df$Involvement <- factor(df$Involvement, levels=c('Peripheral', 'Axial'))
...
for (j in seq_along(dists)) {
  ...
  # create filenames
  filename_plot = paste("bdiv", dists[j], "plot_psa.pdf", sep = "_")
  
  # plot beta diversity
  p <- ggplot() + # data=df, aes(x = PC1, y = PC2, fill = Diagnosis)) +
    geom_point(data = df, aes(x = PC1, y = PC2, color = Involvement),size=4) +
    scale_color_manual(values = c("Peripheral" = col1[1], "Axial" = col1[2])) + #col1, labels = c("Unaffected", "RA")) +
    bkg +
...
# change these names
group.colors <- c(Peripheral = col1[1], Axial = col1[2]) # B3A98C E69F00
plot_data$Group[plot_data$Group == "axial"] <- "Axial"
plot_data$Group[plot_data$Group == "peripheral"] <- "Peripheral"

plot_data$Group <- factor(plot_data$Group, levels = c('Axial', 'Peripheral'))


pdf("/Users/KevinBu/Desktop/clemente_lab/Projects/ampaim/outputs/jobs27/lefse_psa.pdf", width=20, height=20)
