...
out <- glm(shannon_entropy ~ Medication_Status + Diagnosis, data = df, family = gaussian)

summary(out)

df = df[df$Diagnosis %in% c("RA", "psa", "pso"),]
...
fpb = paste(dir, 'alpha_medication_KL2.pdf', sep = "")
